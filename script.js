...
function buildMapData() {
  const items = [];
  let n = 0;
  function push(obj) { obj.id = obj.cat + "-" + (n++); items.push(obj); }
...
function popupFor(item) {
  let html =
    '<span class="pop-badge ' + CATEGORY[item.cat].badge + '">' + esc(item.years) + " · " + esc(CATEGORY[item.cat].label) + "</span>" +
...
function setupMap() {
  if (!window.L) { renderNoMap(); return; }
  buildLegend();

  const items = buildMapData();
  map = L.map("worldMap", { scrollWheelZoom: false, zoomControl: true }).setView([30, 0], 2);
  ...
  items.forEach(function (item) {
    const marker = L.marker([item.lat, item.lng], { icon: pinIcon(item.cat) });
  ...
  function renderCards(list) {
    ...
    if (!list.length) {
      wrap.appendChild(el("li", "map-card map-empty", "No locations in this category yet."));
    }
  }
  ...
  function flyToItem(item, li) {
    ...
    map.flyTo([item.lat, item.lng], Math.max(map.getZoom(), 5), { duration: 0.8 });
  ...
  window.applyMapFilter = function (cat) {
    const list = cat === "all" ? items : items.filter(function (item) { return item.cat === cat; });
    currentItems = list;
    setMapLayers(list);
    renderCards(list);
...
function buildLegend() {
  ...
  Object.keys(CATEGORY).forEach(function (key) {
    legend.appendChild(el("li", null,
      '<span class="dot" style="background:' + CATEGORY[key].color + '"></span>' +
